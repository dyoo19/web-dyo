...
export function DialogDemo({ refetch1 }: DialogProps) {
  ...
  const { mutate } = useMutation({
    mutationKey: ["postAccesories"],
    mutationFn: async () => {
      if (!name || !category || !checkInOut || !image) {
        console.error("Missing required fields");
        return;
      }

      try {
        const { data, error } = await microgen.service("Accessories").create({
          name,
          category,
          checkInOut,
          checkedOutTo,
  ...
  const { data: dataAccesories, refetch } = useQuery(
    "get-Accesories",
    async () => {
      const { data, error } = await microgen.service("AccesoriesAssets").find({
        lookup: { "*": "*" },
        limit: 10,
      });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
          ...
          <span className="font-light">Add Accesories</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="max-h-[95vh] sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Add Accesories</DialogTitle>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              mutate();
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="flex flex-col items-start gap-4">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectGroup>
                      {data.map((accesoriesAsset) => (
                        <SelectItem
                          key={accesoriesAsset.name}
                          value={accesoriesAsset.name}
                          className="font-semibold text-black transition-all duration-150 ease-linear hover:text-white"
                        >
                          {accesoriesAsset.name}
                        </SelectItem>
                      ))}
                    </SelectGroup>
                  </SelectContent>
